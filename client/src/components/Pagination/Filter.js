...
export function Filter({
    ...
    let copy = [...pokemons]
      ...
      .filter((q) => {
        if (filters.type === "All") {
          ...
          return q.types.includes(filters.type);
        }
      });
  ...
  const handleQuitFilters = (filter) => {
    let copy = [...pokemons]
      .sort(sortOptions[sort])
      .filter((p) => {
        if (filter === "type" || filters.type === "All") {
          return true;
        } else {
          return p.types.includes(filters.type);
        }
        //&& p.createdBy === db FALTA APLICAR EL FILTRO PARA CREATED BY
      })
      .filter((q) => {
        if (filter === "db" || filters.db === "All") {
          return true;
        } else {
          return q.createdBy === filters.db;
        }
    ...
    <div className={Container.Bar}>
      ...
      <div className={Container.Dropdown}>
        ...
        >
          ...
          {types &&
            types.sort(sortOptions["AZ"]).map((t) => {
        ...
        {filters.type !== "All" ? (
          <div className={[Button[filters.type], Button.Div].join(" ")}>
            <span>{`${filters.type} `}</span>
            <button
              className={[Button.closeButton, Button[filters.type]].join(" ")}
              onClick={() => handleQuitFilters("type")}
            >
              X
            </button>
          </div>
      ...
      <div className={Container.Dropdown}>
        ...
        {filters.db !== "All" ? (
          <div className={`${Button.Div} ${Button.CloseDiv}`}>
            <span>{`${filters.db} `}</span>
            <button
              className={Button.closeButton}
              onClick={() => handleQuitFilters("db")}
            >
              X
            </button>
          </div>
